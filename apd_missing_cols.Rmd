...
tables = c("collectionunits","sites","datasets","publications", "datasetpublications","constituentdatabases","datasetdatabases")
...
singlepoints = apd_sites_sf %>% dplyr::filter(siteid %in% c(site_coll_singlepoints$siteid)) %>% left_join(apd_colls, by=join_by(siteid)) %>% dplyr::filter(is.na(gpslongitude) & is.na(gpslatitude))
