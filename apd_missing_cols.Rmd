...
tables = c("dslinks","collectionunits","sites","datasets","publications", "datasetpublications","constituentdatabases","datasetdatabases")
...
vanker =dslinks_df %>% dplyr::filter(siteid=="26668") %>% dplyr::left_join(datasets_df) %>% dplyr::left_join(collectionunits_df,by=join_by(collectionunitid))
...
vanker_data %>% group_by(datasetid,datasettype) %>% summarize(toString(unique(variablename))) %>% left_join(counts) %>% datatable(rownames=FALSE)
